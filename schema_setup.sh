...
build_vw_sql="create OR REPLACE view \`${project_id}.${dataset}.log_report\` as WITH source_of_ip_addresses AS (SELECT *, REGEXP_REPLACE(remoteIp, 'xxx', '0')  ip FROM \`${project_id}.${dataset}.log_request_mv\`) SELECT requestUrl,remoteIp,priority,outcome,name,date_timete as date_time,date(date_timete) as date,CNT,city_name,country_name,latitude,longitude,ST_GEOGPOINT(longitude, latitude) AS geopoint FROM (SELECT  *,1 as dummy1  FROM (SELECT *, NET.SAFE_IP_FROM_STRING(ip) & NET.IP_NET_MASK(4, mask) network_bin FROM source_of_ip_addresses, UNNEST(GENERATE_ARRAY(9,32)) mask WHERE BYTE_LENGTH(NET.SAFE_IP_FROM_STRING(ip)) = 4) JOIN \`${project_id}.${dataset}.geolite2_city_ipv4\` USING (network_bin, mask))"
